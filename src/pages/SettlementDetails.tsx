  ...
  const enrichedEmployees = useMemo(() => {
    return employees.map((emp, index) => {
      ...
      if (emp.employment_status === EmployeeStatus.LEAVING || emp.employment_status === EmployeeStatus.LEFT) {
        ...
        // Force some "new" for demo matching the image if data is sparse
        status = 'new';
      } else if (index === 2) {
        // Force one "resigned"
        status = 'resigned';
        ...
        <div className="border border-gray-200 rounded-t-lg bg-white flex flex-col h-full shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {filteredData.map((emp, index) => (
              <div
                key={emp.id}
                className="grid grid-cols-[60px_100px_100px_1fr_1fr_120px_120px] border-b border-gray-100 last:border-0 hover:bg-gray-50 text-sm text-gray-900 items-center"
              >
                ...
                <div className="p-4 flex justify-center">
                  {emp.settlement_status === 'new' && (
                    <Badge className="bg-green-100 text-green-600 hover:bg-green-100 border-0 shadow-none font-medium">
                      신규 입사자
                    </Badge>
                  )}
                  {emp.settlement_status === 'resigned' && (
                    <Badge className="bg-gray-100 text-gray-500 hover:bg-gray-100 border-0 shadow-none font-medium">
                      퇴사자
                    </Badge>
                  )}
                  {emp.settlement_status === 'existing' && <span className="text-gray-300">-</span>}
                </div>
              </div>
            ))}
